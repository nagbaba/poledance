...
function backurl(CompetitionICode, CompetitionCompetitorICode) { window.location.href = base_url+'competition/viewresultpage/'+CompetitionICode+'/'+CompetitionCompetitorICode+'/'; } // go to back page
  ...
  <tr>
    <td height="23" valign="top">
      <table width="98%" border="0" style="background-color:#F3F3F3;">
        ...
        <!--  edited by 4axiz to get the division -->
        <tr>
    ...
    <td valign="top">
    <form name="editpostresult" id="editpostresult" action="<?php echo base_url().'competition/saveupdateresult/';?>" method="post">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td width="271"  valign="top"><table width="98%" height="42" border="0" cellpadding="0" cellspacing="0">
              <tr>
				...
				<?php if(!empty($TitleDetails['ProfileImage'])){?>
      ...
      <tr>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
              ...
              <tr>
                <?php for($k=0; $k < $SectionCount; $k++){
						...
						// get subsection points from table
						$SingleWhere = "CompetitionCompetitorICode = '1' AND SectionICode = '".$row['SectionICode']."' AND SubsectionIcode = '".$Subsectiondetails[$k]['SubsectionICode']."' AND CreatedBy = '".$this->session->userdata('LoginUserICode')."'";
						$editval = $this->home->getSingleFieldValue('post_subsection_result', 'Subsectionsecuredpoints', $SingleWhere);
                    ?>
                     <input type="hidden" name="fieldname[]" value="<?php echo $txtfieldname;?>" />
					 <td height="45" class="<?php echo $Bottomstyle; ?>"><input type="text" id="<?php echo $txtfieldname;?>" name="<?php echo $txtfieldname;?>" style="width:60px;" maxlength="3" onkeyup="return checkMaxScore(this.value,'<?php echo $row['MinPoint'];?>', '<?php echo $row['MaxPoint'];?>', '<?php echo $txtfieldname;?>');"  tabindex="1" <?php echo $disable; ?> value="<?php echo $editval; ?>"/></td>
                <?php }?>
               </tr>
               <tr>
                <td height="19" >&nbsp;</td>
	   ...
	   <tr>
        <td class="table_title">Edit Comment</td>
      </tr>
	  <tr>
        <td><textarea name="Comment" id="Comment" rows="6" cols="50"><?php echo $Comment;?></textarea></td>
      </tr>
	  <tr>
        <td>&nbsp;</td>
      </tr>
      <tr>
        <td><input type="hidden" name="CompetitionICode" id="CompetitionICode" value="<?php echo $CompetitionICode;?>" />
            ...
            <input type="hidden" name="CompetitorICode" value="<?php echo $TitleDetails['CompetitorICode'];?>" /></td>
      </tr>
      <tr>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="89%"><?php if(empty($disable)) {?>
		    <span style = "float:right; width:60px;" class="btn_bx">
		    <input type="button"  value="Update" name="Save" tabindex="2" onclick="return checkCompetitionStatus();"/></span><?php } ?></td>
            <td width="11%"><span style = "float:right; width:60px;" class="btn_bx">
              <input type="button"  value="Close" name="Save" tabindex="3" onclick="return backurl('<?php echo $CompetitionICode;?>', '<?php echo $CompetitionCompetitorICode;?>');"/></span></td>
          </tr>
        </table></td>
      </tr>
      <tr>
