<?php $WhereField = "CompetitionCompetitorICode = '".$CompetitionCompetitorICode."' AND CreatedBy = '".$this->session->userdata('LoginUserICode')."'";
...
function editresult(CompetitionICode, CompetitionCompetitorICode) // for edit result
{ 
	window.location.href = base_url+'competition/editpostedresult/'+CompetitionICode+'/'+CompetitionCompetitorICode+'/'; 
} 
...
<table width="970" border="0">
  ...
  <tr>
    <td height="23" valign="top">
      <table width="98%" border="0" style="background-color:#F3F3F3;">
		...
		<?php if($CompetitionDetails['IsCompleted'] != 1 && $CompetitionDetails['IsStarted'] != 0){ ?>
		<br/>
		<table>
		<tr>
		<td>
          ...
          <td height="29"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                      <tr>
                       <?php
						...
						<?php 
							$SinglefieldWhere = "CompetitionCompetitorICode = '".$CompetitionCompetitorICode."' AND ChampionshipICode = '".$req['ChampionshipICode']."' AND CreatedBy = '".$this->session->userdata('LoginUserICode')."'";
							echo $this->home->getSingleFieldValue('competitor_result', 'SecuredPercentage', $SinglefieldWhere);
						?></b></span></td>
                        <?php } } ?>
                       </tr>
                     </table></td>
                ...
                <?php for($k=0; $k < $SectionCount; $k++){
                    ...
                    ?>
					 ...
					 <td height="45"  class="<?php echo $Bottomstyle; ?>"><span class="table_title">
						<?php 
						$SingleWhere = "CompetitionCompetitorICode = '".$CompetitionCompetitorICode."' AND SectionICode = '".$row['SectionICode']."' AND CreatedBy = '".$this->session->userdata('LoginUserICode')."'";
        ...
        <td><?php if(!empty($Sectiondetails) > 0) {?>
                     <table width="100%" border="0" cellpadding="0" cellspacing="0">
                       ...
                       <tr>
                         ...
                         <?php
                         $Where = "CompetitionCompetitorICode = '".$CompetitionCompetitorICode."' AND SectionICode = '".$row['SectionICode']."' AND CreatedBy = '".$this->session->userdata('LoginUserICode')."'";
  ...
  <tr>
	...
	<input type="hidden" name="CompetitionICode" value="<?php echo $CompetitionICode;?>" />
	...
	<span style = "float:right; width:60px;" class="btn_bx">
      <input type="button"  value="Edit" name="EditResult" onclick="return editresult('<?php echo $CompetitionICode;?>', '<?php echo $CompetitionCompetitorICode;?>');" tabindex="2" /></span></td>
    <td width="4%">&nbsp;</td>
    <td width="8%"><span style = "float:right; width:60px;" class="btn_bx">
      <input type="button"  value="Close" name="Close" onclick="return backurl();" tabindex="1" /></span></td>
  </tr>
</table>
